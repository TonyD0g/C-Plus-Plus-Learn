...
<body>
    ...
    <div class="home">
        <div class="count">
            <a class="box" href="<?php echo U('content/index');?>">
                <h3>内容</h3>
                <div class="num"><?php echo $count['content']['total'];?></div>
                <div class="total">草稿：<span class="red"><?php echo $count['content']['num'];?></span></span></div>
                <div class="icon icon-1"><span class="ui-icon-file-word"></span></div>
            </a>
            <a class="box" href="<?php echo U('book/index');?>">
                <h3>留言</h3>
                <div class="num"><?php echo $count['book']['total'];?></div>
                <div class="total">待处理：<span class="red"><?php echo $count['book']['num'];?></span></div>
                <div class="icon icon-3"><span class="ui-icon-solution"></span></div>
            </a>
			<a class="box" href="<?php echo U('link/index');?>">
                <h3>友链</h3>
                <div class="num"><?php echo $count['link']['total'];?></div>
                <div class="total">草稿：<span class="red"><?php echo $count['link']['num'];?></span></div>
                <div class="icon icon-2"><span class="ui-icon-link"></span></div>
            </a>
            <a class="box checkupdate" href="javascript:;">
                <h3>版本</h3>
                <div class="num"><?php echo C(strtoupper('version_number'));?></div>
        ...
        <div class="ui-row ui-mt-20 homeother">
            ...
            <div class="ui-col-6">
                 <div class="box">   
                    <div class="subject"><a href="<?php echo U('loglogin/index');?>">更多>></a>登录日志</div>
                    <div class="log">
                        <?php $array_rs=$this->db->load("select * from cms_log_login  where $where  order by id desc limit 20",0,false,0);$total_rs=count($array_rs);if($total_rs==0){ ?>
<?php } else{ ?>
<?php $i=0;} foreach($array_rs as $rs){ $i++;?>
                        <p><strong><span><?php echo date('Y-m-d H:i',$rs['logindate']);?></span><?php echo $rs['loginname'];?></strong><?php echo $rs['loginip'];?><span><?php echo $rs['loginmsg'];?></span></p>
                        <?php } if($total_rs>0){ ?>
<?php }?>
                    </div>
                </div>
