    	...
    	<div class="container-left">
        	<div class="box">
                ...
                <ul class="ui-media-list ui-mt-30 ui-mb-10">
                	{cms:rs pagesize="20" table="$table" where="$where" order="$order"}
                    {rs:eof}没有找到含有<span class="ui-text-red ui-ml ui-mr">{$keyword}</span>的内容{/rs:eof}
                    <li class="ui-media">
                        {if $rs[ispic]==1}
                        ...
                        <div class="ui-media-body">
                            <div class="ui-media-header ui-text-hide"><a href="{$rs[link]}" target="_blank" title="{$rs[title]}">{redkey($rs[title],$keyword)}</a></div>
                            <div class="ui-media-text">{redkey(nohtml($rs[intro]),$keyword)}</div>
