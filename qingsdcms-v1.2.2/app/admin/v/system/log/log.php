...
<body>
    <div class="position">当前位置：系统管理 > <a href="{THIS_LOCAL}">管理日志</a></div>
    <div class="border">
        <!---->

        <a href="javascript:;" class="btach ui-btn ui-btn-info"><i class="ui-icon-delete"></i>批量删除</a>
        ...
        <table class="ui-table ui-table-border ui-table-hover ui-table-striped ui-mb ui-mt-20">
            ...
            <tbody>
            {cms:rs pagesize="20" table="cms_log" where="$where" order="id desc"}
            {rs:eof}
            <tr>
            ...
            <tr>
                ...
                <td>{$rs[id]}</td>
                <td>{$rs[title]}</td>
                <td class="ui-text-left ui-text-hide">{$rs[url]}</td>
                <td>{$rs[msg]}</td>
                <td>{$rs[ip]}</td>
